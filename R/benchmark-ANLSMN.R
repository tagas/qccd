...
ext_all <- c("LS-s/")#c("LS-s/", "AN-s/", "LS/", "LS-s/", "MN-U/")
  ...
  for(ext in ext_all){
    ...
    for(i in 1:n_pairs) {
      ...
      pair <-  read.table(paste0(path, ext, "pair_",i,".txt"),
                          ...
                          row.names = 1)
      plot(pair)
      res <-method_to_run(pair)## method_to_run(cbind(pair[,2], pair[,1]))
      t2 <- Sys.time()
    ...
    write.table(resDF, file = paste0("results/",method,"_",substr(ext, 1, nchar(ext) - 1),"_sept_m1.tab"),
                row.names = F,
                quote = F,
                sep = "\t")
...
methods <- list("CAM")#, "CAM", "RESIT", "EMD",  "LINGAM", "Slope") 
